...
var lastStoneWeight = function (stones) {
    const createHeap =
        ...
        () => {
            ...
            const createTransferConditionWhenInsert =
                ({ max, min }) =>
                ({ node, parent }) => {
                    if (min) {
                        return node < parent
            ...
            const canTransferWhenInsert = createTransferConditionWhenInsert({
                max,
                min,
            })
            const createTransferConditionWhenRemove =
                ({ max, min }) =>
                ({ node, left, right }) => {
                    if (min) {
            ...
            const canTransferWhenRemove = createTransferConditionWhenRemove({
                max,
                min,
            })
            const createGetterOfSwapTarget =
                ({ max, min }) =>
                ({ left, right }) => {
                    if (max) {
            ...
            return {
                ...
                remove: () => {
                    ...
                    while (
                        canTransferWhenRemove({
                            node: heap[idx],
                            left: heap[left],
                            right: heap[right],
                        })
                    ) {
                        const swapTarget = getSwapTarget({
                            left: heap[left],
                            right: heap[right],
                        })
