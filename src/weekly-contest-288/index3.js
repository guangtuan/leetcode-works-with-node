...
var maximumProduct = function (nums, k) {
    const createHeap = () => {
        ...
        const ifTransferWhenRemove = ({ node, left, right }) =>
            node >= left || node >= right
        const eleIsNull = (value) => value === undefined
        const eleIsNotNull = (value) => value !== null && value !== undefined
        const getSwapTarget = ({ left, right }) =>
            Math.min(...[left, right].filter(eleIsNotNull))
        return {
            inner: () => heap.slice(1),
            remove: () => {
                if (heap.length === 1) {
